...
class apiController extends appController
{
    ...
    public function user_reset_password()
    {
        ...
        if ($uid == uid()) {
            return self::send_error(LR_API_ARGS_ERROR, __('API_MESSAGE_CANNOT_RESET_OWN_PASSWORD'));
        }
    ...
    public function user_update_profile()
    {
        ...
        if (!not_empty($email)) {
            return self::send_error(LR_API_ARGS_ERROR, __('INPUT_CHECK_BAD_ARGS', 'EMAIL'));
        }
        if (!not_empty($mobile)) {
            return self::send_error(LR_API_ARGS_ERROR, __('INPUT_CHECK_BAD_ARGS', 'MOBILE'));
        }
    ...
    public function user_level()
    {
        ...
        if ($uid == uid()) {
            return self::send_error(LR_API_ARGS_ERROR, __('API_MESSAGE_CANNOT_CHANGE_OWN_LEVEL'));
        }
        ...
        if (db_errno() != 0) {
            ...
            if ($level == 0) {
                ...
                if ($level == 0) {
                    $level_text = __('ACCOUNT_CLOSED');
                } elseif ($level == 9) {
                    $level_text = __('ACCOUNT_SUPER_ADMIN');
                } else {
                    $level_text = $level;
                }
    ...
    public function todo_remove_comment()
    {
        $hid = intval(v('hid'));
        if (intval($hid) < 1) {
            return self::send_error(LR_API_ARGS_ERROR, __('INPUT_CHECK_BAD_ARGS', 'HID'));
        }
    ...
    public function feed_remove_comment($cid = flase)
    {
        $cid = intval(v('cid'));
        if (intval($cid) < 1) {
            return self::send_error(LR_API_ARGS_ERROR, __('INPUT_CHECK_BAD_ARGS', 'CID'));
        }
    ...
    public function feed_remove($fid = false)
    {
        ...
        if (intval($fid) < 1) {
            return self::send_error(LR_API_ARGS_ERROR, 'FID NOT EXISTS');
        }
    ...
    public function im_send($uid = false, $text = false)
    {
        ...
        if ($uid == uid()) {
            return self::send_error(LR_API_ARGS_ERROR, __('API_MESSAGE_SPEAK_TO_SELF'));
        }
    ...
    public function im_history($uid = false)
    {
        ...
        */
        ...
        $word = z(t(v('word')));
        if (strlen($word) > 0) {
            $wsql .= $wsql." AND `content` LIKE '%".s($word)."%' ";
        }
